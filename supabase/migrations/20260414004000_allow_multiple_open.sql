-- Migration: Allow Multiple Open Puzzles
-- Removes the 'auto-fail' logic when skipping puzzles.
-- Allows the user to have multiple puzzles with status NULL (unsolved) in their history.
...
BEGIN
    -- 1. Get current active puzzle ONLY if p_force_new is FALSE
    -- If p_force_new is TRUE, we skip this to generate a new entry
